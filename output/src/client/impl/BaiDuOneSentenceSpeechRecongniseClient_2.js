...
var BaiDuOneSentenceSpeechRecongniseClient = /** @class */ (function () {
    ...
    BaiDuOneSentenceSpeechRecongniseClient.prototype.startHandleSingleVoice = function (_a) {
        ...
        return __awaiter(this, void 0, void 0, function () {
            var translateTextArr, translatePath, rs_1, translateTextPath, rs, timeQuanTum, tasksP, _loop_1, this_1, index, len, rsList, translateTextArr, _i, rsList_1, ele, result, translateTextPath;
            var _this = this;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 3:
                        ...
                        if (!(timeQuanTum.length > 1)) return [3 /*break*/, 5];
                        tasksP = [];
                        _loop_1 = function (index, len) {
                            var startTime = timeQuanTum[index];
                            var nextTime = timeQuanTum[index + 1];
                            var duration = TimeUtils_1.TimeUtils.getMinDuration(startTime, nextTime);
                            var srcPath = absolutePath;
                            var divisionPath = this_1.divisionCachePath + '\\' + fileNameExcludeSuffix + '_division_' + index + '.' + suffix;
                            var nextPath = divisionPath;
                            //division
                            var taskP = this_1.divisionVoiceByTime({ startTime: startTime, duration: duration, srcPath: srcPath, divisionPath: divisionPath })
                                .then(function (rs) {
                        ...
                        this_1 = this;
                        for (index = 0, len = timeQuanTum.length; index < len - 1; index++) {
                            _loop_1(index, len);
                        }
                        return [4 /*yield*/, Promise.all(tasksP)];
                    case 4:
                        rsList = _b.sent();
                        translateTextArr = [];
                        for (_i = 0, rsList_1 = rsList; _i < rsList_1.length; _i++) {
                            ele = rsList_1[_i];
                            result = ele.result;
                            if (result && result[0]) {
    ...
    BaiDuOneSentenceSpeechRecongniseClient.prototype.saveTranslateTextToFile = function (_a) {
    ...
    BaiDuOneSentenceSpeechRecongniseClient.prototype.getVoiceLen = function (voice) {
